...
app.get('/getChalanNo', async (req, res) => {
    try {
        ...
        if (count) {
            ...
            const newCount = new billCount({ count: 1, date: new Date() });
            await newCount.save();
            return res.status(200).json({ data: newCount });
        }
    } catch (err) {
...
app.get('/getBaleNo', async (req, res) => {
    try {
        ...
        if (count) {
            ...
            const newCount = new baleCount({ count: 1, date: new Date() });
            await newCount.save();
            return res.status(200).json({ data: newCount });
        }
    } catch (err) {
